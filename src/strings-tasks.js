...
function removeFirstOccurrences(str, value) {
  ...
  if (str.indexOf(value) >= 0) {
    const part1 = str.slice(0, str.indexOf(value));
    const part2 = str.slice(str.indexOf(value) + value.length);
    result = part1 + part2;
  }
  return result;
}
...
function removeLastOccurrences(str, value) {
  ...
  if (str.lastIndexOf(value) >= 0) {
    const part1 = str.slice(0, str.lastIndexOf(value));
    const part2 = str.slice(str.lastIndexOf(value) + value.length);
    result = part1 + part2;
  }
  return result;
}
...
function countVowels(str) {
  ...
  vowels.forEach((el) => {
    let place = -1;
    let whileStr = str;
    while (whileStr.indexOf(el) >= 0) {
      count += 1;
      place = str.indexOf(el);
      whileStr = whileStr.slice(place + 1);
    }
  });
  return count;
  // throw new Error('Not implemented');
...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  if (str.length > 1) {
    const strChange = str.replace(/,|\s|!|\?/g, '').toLowerCase();
    const lastPart = strChange
      .slice(Math.ceil(strChange.length / 2))
      .split('')
      .reverse()
      .join('');
    return strChange.slice(0, Math.floor(strChange.length / 2)) === lastPart;
  }
  return true;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (letter) => rot[alpha.indexOf(letter)]);
}
